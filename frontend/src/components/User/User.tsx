...
function User({ user, chats, addFriend, handleActiveChat, activeChat }: IUser) {
  ...
  return (
    <div
      className="w-full h-16 cursor-pointer mt-1 flex p-2 justify-between items-center bg-dark-gray rounded-lg overflow-hidden hover:opacity-[75%]"
      onClick={() =>
    ...
    >
      ...
      {isChatExist ? (
        ...
        >
          <UserAddOutlined className="text-blue-700 text-opacity-80 hover:text-opacity-[100%] hover:text-blue-500" />
        </button>
      )}
    </div>
  );
